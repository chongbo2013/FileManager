...
/**
 * This class defines a method for searching images from database.
 *
 * @author wangfan
 * @version 2012.07.04
 */
...
public class ImageDB {
    ...
    //uri for searching images in database
    private static Uri uri = Constants.URI_IMAGE;
    ...
    public static List<ImageFileInfo> list = new ArrayList<ImageFileInfo>();

    /**
     * This method search matched image files
     *
     * @param key      key in searching images
     * @param sortMode defines how to sort the list
     */
    public static List<ImageFileInfo> searchImages(String key, String sortMode) {
        //load thumbnail
        thumbnail = SoftCache.imageThumb;
            ...
            do {
                ...
                if (loadThumb == true)    //create new item with thumbnail
                {
                    if (title.toLowerCase().contains(key.toLowerCase()) || text.toLowerCase().contains(key.toLowerCase())) {
                        normalItem = new ImageFileInfo(title, text, thumbnail, filePath, Constants.CLASS_IMAGE);
                ...
                {
                    if (title.toLowerCase().contains(key.toLowerCase()) || text.toLowerCase().contains(key.toLowerCase())) {
                        normalItem = new ImageFileInfo(title, text, null, filePath, Constants.CLASS_IMAGE);
