...
//                    return new DrawableHolder();
    ...
    private boolean loadCachedIcon(ImageView view, String path, FileCategory cate) {
        ...
        if (holder == null) {
            ...
            if (Util.imageViewTagIsEqual(view, path) && holder.setImageView(view)) {
                return true;
            }
        }
    ...
    public long getDbId(String path, boolean isVideo) {
        ...
        {
        	 Toast.makeText(mContext, R.string.sdcard_io_error, Toast.LENGTH_SHORT).show();
        }
        if (c == null) {
            return 0;
        }
                ...
                if (holder != null && holder.state == ImageHolder.NEEDED) {
                    ...
                    switch (id.mCategory) {
                        case Apk:
                            ...
                            break;
                        case Music:
//                            Bitmap iconMusic = Util.getMusicThumbnail(id.mPath);
//                            holder.setImage(iconMusic);
                            break;
                        case Picture:
                        	if (id.mId == 0)
...
//                        	Bitmap pic = getImageThumbnail(id.mId);
//                        	if(id.mId != 0 && pic != null){
//                        	holder.setImage(pic);
//                        	} else{
//                        	Log.d("yx", "IconLoader -> run -> picture dbthum:" + getImageThumbnail(id.mId));
                            holder.setImage(ThumbnailUtils.createImageThumbnail(id.mPath, MICRO_KIND));
//                        	}
                        	break;
                        case Video:
//                            boolean isVideo = id.mCategory == FileCategory.Video;
                            if (id.mId == 0)
                                id.mId = getDbId(id.mPath, true);
                            if (id.mId == 0) {
                                Log.e("FileIconLoader", "Fail to get dababase id for:" + id.mPath);
                            }
                            // #944352 add begin by bin.dong
                            Bitmap pic2 = /* getVideoThumbnail(id.mId) */ThumbnailUtils
                                    .createVideoThumbnail(id.mPath, MICRO_KIND);
                            // #944352 add end by bin.dong
                            holder.setImage(pic2);
                            /*
                             * if(id.mId != 0 && pic2 != null){
                             * holder.setImage(pic2); } else{ Log.d("yx",
                             * "IconLoader -> run -> video dbthum:" +
                             * getImageThumbnail(id.mId));
                             * holder.setImage(ThumbnailUtils
                             * .createVideoThumbnail(id.mPath, MICRO_KIND)); }
                             */
                            break;
                        case Theme:
//                        	Log.d("yx", "IconLoader -> handleMessage: theme");
                        	Drawable themeIcon = Util.getThemeIcon(mContext, id.mPath);
                            holder.setImage(themeIcon);
                        	break;
                    }
...
//        	Log.d("yx", "IconLoader -> getImageThumbnail");
            return Images.Thumbnails.getThumbnail(mContext.getContentResolver(), id, MICRO_KIND, null);
        }

        private Bitmap getVideoThumbnail(long id) {//device reboot here......NOTICE!!!!!!!!!
